...
static void send_output_tokenize(arith_token_tokenize_result* dst, arith_token token, bool* unary_allowed_next) {
  ...
  switch (token.type) {
    ...
    case ARITH_TOKEN_BITWISE_COMPLEMENT:
      *unary_allowed_next = false;
      break;
    default:
      *unary_allowed_next = true;
      break;
  }
}

// send a 0 before send_output_tokenize.
// the shunting yard method can't handle unary, so all unary is converted to a binary op
static void send_zero_before_unary(arith_token_tokenize_result* dst, arith_token unary, bool* unary_allowed_next) {
  arith_token zero;
  zero.type = ARITH_TOKEN_U32;
  zero.begin = unary.begin;
  zero.value.u32 = 0;
  send_output(dst, zero);
  send_output_tokenize(dst, unary, unary_allowed_next);
}
...
void tokenize_arithmetic_expression(const CODE_UNIT* begin, const CODE_UNIT* end, arith_token_tokenize_result arg) {
  assert(begin <= end);
  // given what was read so far, is an operator an unary op?
  bool accepting_unary = true;
  while (begin != end) {
    CODE_UNIT ch = *begin;
...
begin_iter:
    switch (ch) {
      ...
      case '-': {
        ...
        send_zero_before_unary(&arg, token, &accepting_unary);
      } break;
      case '~': {
        arith_token token;
        token.begin = begin;
        if (!accepting_unary) {
          // normally this is out of scope of the tokenizer (instead done in the
          // parser), but it was easiest to do this here
          token.type = ARITH_TOKEN_ERROR;
          token.value.error_msg = "'~' unary operator mustn't be proceeded by a value";
...
simple_token:
        ...
        send_output_tokenize(&arg, token, &accepting_unary);
      } break;
      case '<':
      case '>': {
        bool is_lt = ch == '<';
